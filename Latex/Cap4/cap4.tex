...
developed by the experiment. In this chapter the reconstruction of the interested objects are described. I have contributed in the evaluation of the best working point for Standard Model Higgs boson analysis with 2015 data~\cite{CMS-PAS-HIG-16-021} and marginally for the same analysis using 2016 data~\cite{CMS-PAS-HIG-16-042}, Tab.~\ref{btt}.}
...
The DA process finds not only positions and assignments of tracks to vertices but also thex number of vertices.
...
\label{jetr}
Jets are the experimental signature of quarks and gluons produced in the hadron collision. They result from the parton  hadronization and they play a major role in a hadronic collider where final state with jets have very large cross-section. %The technique used for jet reconstruction is the Particle Flow.
...
\end{itemize}
Jet definition is not unique, being the parton not a well-defined object, from the experimental point of view, so several
