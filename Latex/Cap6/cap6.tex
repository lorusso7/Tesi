...
\caption{ 
    Distributions of the MC generator level for signals (no possible reconstruction), for $m_T^H$, for $m_{\ell \ell}$ and for  $m_T^I$ variables using 
different values of $C'$ (1,0.5,0.1).}
    \label{fig:cprime}
\end{figure}
...
\section{Fully and same leptonic results}
...
\end{figure}\\
\newline
The same procedure is adopted to evaluated cross section limits  for the same leptonic final state, Fig~\ref{limit_observed_lnuqq_VBF0_log-1}.
\begin{figure}[htb]
\centering
\includegraphics[width=0.65\textwidth]{../Cap6/limit_observed_lnuqq_VBF0_log-1}
...
\end{figure}
The fully and same leptonic analysis are complementary: the former puts a powerful limit at low mass ($<$ 1 TeV), the other instead at very high mass.
To perform a right comparison among the two limits,  Fig.~\ref{ATLAS-CONF-2016-074_fig} and  Fig~\ref{limit_observed_lnuqq_VBF0_log-1}, the 
...
Using the EW model interpretation, the  combination of the fully leptonic and semi leptonic analysis has been performed for cross section limits, Fig.~\ref{lim_superC}. The limit result represents a  improvements for the high mass searches. 
...
\end{figure}\\
\newline
Also the MSSM limits  in function of $m_A$ are computed for the combination among the two analysis, Fig.~\ref{fig:MSSM2}.
\begin{figure}[htb]
\centering
\subfigure[$m_h^{mod+}$]{
...
In particular, the 2HDM limits for both the type-1 and the type-2 models are displayed in a $\cos(\beta-\alpha)$-$\tan\beta$ plane,  Fig.~\ref{fig:2HDM1}, 
...
and 2HDM type-I and type-II scenarios.  
