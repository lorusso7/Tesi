...
\textit{In the search of a high mass Higgs boson described in this thesis, no excesses beyond the SM expectations are found. Therefore, upper limits on signal production are computed. This chapter presents the 95\% CL exclusion limits
...
\caption{95$\%$ CL exclusion limits on a 2HDM versus $\cos(\beta-\alpha)$ on the x-axis. Limits are shown for a type-1 and type-2 2HDM for different masses $m_{H}=200$, $300$, $500\,$GeV.}
    \label{fig:2HDMa}
\end{figure}
...
\caption{95$\%$ CL exclusion limits on a 2HDM versus $m_{H}$ on the x-axis. Limits are shown for a type-1 and type-2 2HDM for $\cos(\beta-\alpha)=0.1$.}
    \label{fig:2HDMb}
\end{figure}
...
the former sets a powerful limit at low mass ($<$ 1 TeV), the other at  high mass\footnote{To correctly compare among the limits in  Fig.~\ref{fig:lim_OFSF_comb} and  Fig~\ref{limit_observed_lnuqq_VBF0_log-1}, the 
fully leptonic ones must be multiplied by a factor of $\sim$10 to take in account the BR in $ 2\ell 2\nu$.\\
