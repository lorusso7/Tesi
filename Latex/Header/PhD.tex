...
\chapter{Lepton  Efficiencies from Tag and Probe Method}
\label{TP}
...
as electron. After passing some kinematical selection, we form the total probe selection. The invariant 
mass of the pair is then reconstructed from Tag and Probe electron and a Z boson mass
...
difference in efficiencies between data and MC. 
The Pile-Up reweighting is also applied on MC during the computation of efficiencies. 
A MC truth matching has also been applied in case of
the computation using simulation. There are basically two methods to estimate the efficiencies.
One is the \textit{counting method} and the other is the \textit{fitting method}. The counting method is used
when there is less background. We are computing efficiencies with loose Z mass window and
hence there is a possibility of background contamination under the Z peak. 
Hence, proper handling of background becomes important. In order to take into account the effect of background,
the fitting method has been used to estimate the efficiency.

The Drell-Yan Monte Calro sample is used as a default one. Tag and Probe pairs are then selected using Z mass window of 60 to 120 GeV.
...
For signal
fitting, MC templates are derived from the simulated sample in the Z mass range 60 to 120
GeV in  $p_T$ and $\eta$ bins.
The MC Templates are then smeared using Gaussian function. For the
background fitting a function, that is combination of
...
\subsection*{Muons} 
The muon ID and Isolation efficiency is studied and compared to the prediction of MC in order
to understand if a correction is needed. The muon efficiency is obtained as

\begin{equation}
\epsilon_{\mu} = \epsilon_{TRK} \times \epsilon_{TightHWW} \times \epsilon_{ISO_Tight} \; ,
\end{equation}
\newline
where $ \epsilon_{TRK}$ is the tracker (TRK) muon efficiency.  
The $ \epsilon_{TightHWW} $ muon efficiency
under the assumption that the muon passes the selections summarized in  Tab.~\ref{IDm}.
...
The $\epsilon_{TightHWW}$ and $ \epsilon_{ISO_Tight}$ are determined  using the tag-and-probe method. 
The tag muon is obtained by applying  the kinematic selections and in addition 
 is required to match to the trigger using the isolated muon with $p_T>20$ GeV.
The probe muon  is requested to pass the selection and the isolation described.
...
isolation in the double lepton trigger compared to single muon trigger. Their values on the
plateau are around 93\%-99\%.
