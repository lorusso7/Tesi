...
For AK8 jets the softdrop algorithm~\cite{Larkoski2014} is implemented to suppress the effects of pileup and underlying event radiation. This technique declusters the jet and recursively removes soft wide-angle radiation, pushing QCD jets mass towards lower values while maintaining the jet mass for V-jets around the boson-mass. Each AK8 jet has two associated subjets corresponding to the protojets obtained by undoing the last iteration of the soft drop jet declustering procedure. These protojets are related to the n-subjettiness variable~\cite{Thaler:2011gf}, that is used to quantify the compatibility of AK8 jets with a substructure hypothesis of n-subjets. It is based on the distribution of the
...
%a standard candle. For those backgounds containing QCD jets we apply JMS and JMR uncertainties based on varying the mass value and its resolution by the measured uncertainties for boosted
%W's. 
