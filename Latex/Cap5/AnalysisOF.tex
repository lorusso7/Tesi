...
The events are subdivided in signal and control regions. The signal events are expected to fill to first one. Instead the second ones are expected to contain mainly 
backgrounds events and are used to normalized the backgrounds shape.\\
\newline
...
by defining a background-only control region and only looking at data events
falling in such a region. The control region is precisely defined according to selection
criteria that exclude the signal events. 
Data in the signal region were not examined until the event selection
criteria were decided only relying on Monte Carlo simulations. 
Only when data distributions in signal-depleted regions confirmed the robustness of the analysis, data in
signal region were inspected. 
...
and isolated, to reject non-prompt leptons and misidentified (``fake'') leptons. To suppress
...
be present. The low dilepton invariant mass region 
%dominated by QCD production of leptons 
is not considered in the analysis and $m_{\ell \ell}$  is requested to
...
signal is searched for a cut on $m_T^I >$ 100 GeV is applied.
...
to be identified as b-jets according to the CMVA tagger, loose WP.
...
\item  $m_T^H >$ 60 GeV;
\item  no b-tagged (CMVA loose WP) jets with $p_T >$  20 GeV;
\end{itemize}
...
\item {\bf 2 jet}, exacly 2 jets are required in the event and in addition the condition $\Delta \eta_{jj} < 3.5$ {\bf or} $m_{jj} <$ 500 GeV must hold;
\item {\bf VBF}, exacly 2 jets are required in the event and in addition the condition $\Delta \eta_{jj} > 3.5$ {\bf and} $m_{jj} >$ 500 GeV must hold;
\end{itemize}
All categories, including the 2 jet and the VBF ones, are mutually exclusive by construction.
To extract the high mass boson signals in these four categories, the  strategy  is the following: the $m_T^I$ distribution is fitted by the sum of
...
top Monte Carlo events in each bin of the template, since this simulated sample has the lower statistics with respect to collected luminosity.
The chosen bins are: 
\begin{itemize}
...
\caption{Unblinded distributions of $m_T^I$ in the signal region for 0, 1, 2 and VBF categories. The signal hypotheses correspond to $m_X $ of 2 and 3 TeV.}
...
possible contribution from non-prompt leptons (lower edge) and from  $t \bar{t}$ (higher edge).
...
\caption{Control plots for several variables in a Drell-Yan enriched phase space for VBF events.}
...
\item {\bf 2 jet}, exacly 2 jets with at least one of them b-tagged and in addition the condition $\Delta \eta_{jj} < 3.5$ {\bf or} $m_{jj} <$ 500 GeV must hold;
\item {\bf VBF}, exacly 2 jets with at least one of them b-tagged and in addition the condition $\Delta \eta_{jj} > 3.5$ {\bf and} $m_{jj} >$ 500 GeV must hold.
\end{itemize}
A jet is considered b-tagged if its CMVA score is above the threshold fot the loose working point.
